...
function getAiChoiceEasy(){
    ...
    }while (typeof gameBoard.getBoard()[i] != "number")
    return i;
}

function markAiChoiceEasy(){
    let choice = getAiChoiceEasy();
    roundDetails.setTurns();
    gameBoard.update(choice, "O");
    gridCells[choice].textContent = "O";
}
...
function playRound(e) {
    if (e.target.textContent!= ""){
      ...
      setTimeout(() =>{
          ...
          if (n == 0){
            markAiChoiceHard()
          }else{                    /* Has a chance to make a random choice. Otherwise, it picks a cell using the minimax algorithm*/
            markAiChoiceEasy();
          };                        /* Add delay to computer's choice for extra realism */
